...
def train_ex(inp, outp, model, lr=0.01, this_test_set=[["rOau",".rO.a.u."], ["axxaO",".a.xa.O."], ["rxxa",".xa."], ["axrxu",".a.xu."], ["ttxaO",".xa.O."], ["Ouuut",".O.u.u.u."], ["rutOu",".ru.tO.u."], ["tOaux",".tO.a.u."], ["xttax",".ta."], ["xuxut",".xu.xu."]]):
    ...
    logits = model([inp])[1]
    for index, char in enumerate(list(outp) + ["EOS"]):
        loss += criterion(logits[index][0],torch.LongTensor([model.char2ind[char]]))
    
    loss.backward()
        
    optimizer.step()
    
    preds = [[to_eos(model([pair[0]])[0][0]),to_eos(pair[1])] for pair in this_test_set]
